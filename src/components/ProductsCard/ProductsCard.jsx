...
function ProductsCard({ id, title, image, price, discont_price }) {
  ...
  // Determine the page name based on the current location
  const getPageName = () => {
  ...
  return (
    ...
    >
      <Link to={`/product/${id}`} state={{ from: location.pathname, pageName: getPageName() }} className={styles.card}>
        <img
          src={`http://localhost:3333${image}`}
          alt={title}
          className={styles.cardImg}
